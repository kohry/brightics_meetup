...
def _is_dummy(size) :
    if (size == 476625) : return True #달보드레
    if (size == 33047) : return True #쓰다듬어주세요
    return False
...
def __get_content(site, url, title) :
    ...
    def ___get_image_content(dom) :
        ...
        outfile = str(randint(0, 1000))
        urllib.request.urlretrieve(image_href, outfile)

        #특정 이미지는 저장할필요가 없다. (짤방 등)
        with open(outfile, 'rb') as my_file:
            image_file_size = _get_size(my_file)
            if _is_dummy(image_file_size) :
                return

        image_list.append(image_href)
        upload_storage_image_key = __upload_image_storage(outfile, title)
...
def __upload_image_storage(outfile,title) :
    st = storage.bucket()
